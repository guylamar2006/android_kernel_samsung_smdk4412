...
static int mdm_reset_notify_main(struct notifier_block *this,
	...
	struct mdm_hsic_pm_data *pm_data = get_pm_data_by_dev_name("mdm_hsic_pm0");
	struct device *hdev;
	struct s5p_ehci_hcd_stub *s5p_ehci;
	struct usb_hcd *hcd;

	pr_info("%s\n", __func__);

	hdev = pm_data->udev->bus->root_hub->dev.parent;
	s5p_ehci = dev_get_drvdata(hdev);
	hcd = s5p_ehci->hcd;
