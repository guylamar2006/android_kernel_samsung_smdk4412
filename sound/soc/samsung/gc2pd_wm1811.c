...
#if defined(CONFIG_TARGET_LOCALE_KOR) && defined(CONFIG_MACH_GC2PD)
			msleep(250);
#else
			msleep(100);
#endif
	...
	struct snd_pcm_hw_params *params)
{
	...
	dev_info(codec_dai->dev, "%s ++, stream: %d(p0, c1) prate: %d\n",
			__func__, substream->stream, params_rate(params));
					...
					struct snd_pcm_hw_params *params)
{
	...
	if (aif2_mode == 0) {
		fll_src = WM8994_FLL_SRC_BCLK;
		freq_in = bclk;
		freq_out = prate * 256;

		ret = snd_soc_dai_set_pll(codec_dai, WM8994_FLL2,
					fll_src, freq_in, freq_out);
	} else {
		fll_src = WM8994_FLL_SRC_MCLK1;
		freq_in = GC2PD_DEFAULT_MCLK1;
		freq_out = prate * 256;
	...
	if (wm8994->fll_byp && fll_src == WM8994_FLL_SRC_BCLK &&
